...
import { createLeaveRequest, deleteLeaveRequest, getAllLeaveRequests, getLeaveRequestsByUser, updateLeaveRequest } from '../../services/moduleA/leaveRequestService';
...
const LeaveRequestManagement = () =>
{
  ...
  const loadLeaveRequests = async () =>
  {
    try
    {
      ...
      if (filterUserId)
      {
        const data = await getLeaveRequestsByUser(parseInt(filterUserId), fromDate, toDate);
      ...
      {
        setInfo('No leave requests found');
      }
    } catch (err)
    {
    ...
    <div className="payroll-list-container p-4">
      ...
      <Card className="shadow-sm">
        <Card.Header className="bg-light">
          <h6 className="mb-0">Leave Requests ({leaveRequests.length})</h6>
        </Card.Header>
                ...
                {leaveRequests.map(lr =>
                {
                  ...
                  return (
                    <tr key={lr.id}>
                      ...
                      <td>{getDeptName(lr.user?.departmentId)}</td>
                      <td>{lr.startDate}</td>
                      <td>{lr.endDate}</td>
                      <td>{days}</td>
                      <td>
                        <span className={`badge bg-${lr.status === 'approved' ? 'success' : lr.status === 'rejected' ? 'danger' : 'warning'}`}>
                          {lr.status?.charAt(0).toUpperCase() + lr.status?.slice(1) || 'Pending'}
                        </span>
                      </td>
